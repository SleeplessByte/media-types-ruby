  ...
  class Scheme
    class Links
      ...
      def link(key, allow_nil: false, optional: false, &block)
        ...
        raise DuplicateKeyError, "A link with the same string representation as the string '#{key}' already exists. Please remove one of the two." if rules.has_key?(String(key).to_sym)
        links.add(
          key,
